...
async function createCard(data, section, newSearch) {
    ...
    toAlbum.innerText = data.album_type === 'album' || 'compilation' ? "Add Album" : "Add Single";
...
// ------ Trakcs Templates
async function createCardForTracks(data, section, newSearch) {
    // Await Genre from artist fetch as genre does not exist on new releases
    let genre = await fetchArtistGenre(spotifyToken, data.artists[0].id)
